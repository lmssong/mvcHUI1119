...
function createMenuItem(menus) {
    if (menus.length == 0) { return;}
    var topWindow = $(window.parent.document),
        menuStr = topWindow.find('#menuLeft');
    for (var i = 0; i < menus.length; i++) {
        var itemMenu = menus[i];
        menuStr.append('<dl ' +
            +'id = "' + itemMenu.menuNo + '" >' +
            + '<dt> <i class="Hui-iconfont">&#xe616;</i> "' + itemMenu.text + '" < i class= "Hui-iconfont menu_dropdown-arrow" >&#xe6d5;</i >' +
            + '</dt> ' +
            +'<dd>'+
            +'<ul>' +
            +'<li><a data-href="' + itemMenu.menuUrl + '" data-title="' + itemMenu.text + '" href="javascript:void(0)">"' + itemMenu.text + '"</a></li>' +
            +'</ul>' +
            +'</dd>' +
            +'</dl>');
    }
